...
(function() {
    ...
    applyToShareJS = function(editorDoc, delta, doc) {
        ...
        pos = getStartOffsetPosition(delta);
        var len = delta.lines.length;
        var newline = editorDoc.getNewLineCharacter();
        var newlineLength = newline.length;

        if (delta.action === "insert" && len === 1) {
            doc.insert(pos, delta.lines[0]);
        } else if (delta.action === "insert" && len > 1) {
            var text = "";
            for (var i = 0; i < len; i++) {
                text += delta.lines[i] + (i === len - 1 ? "" : newline);
            }
            doc.insert(pos, text);
        } else if (delta.action === "remove" && len === 1) {
            doc.del(pos, delta.lines[0].length);
        } else if (delta.action === "remove" && len > 1) {
            var cnt = 0;
            for (var i = 0; i < len; i++) {
                cnt += delta.lines[i].length + (i === 0 ? 0 : newlineLength);
            }
            doc.del(pos, cnt);
        } else {
            console.error("Unknown action" + delta.action);
        }
    };
